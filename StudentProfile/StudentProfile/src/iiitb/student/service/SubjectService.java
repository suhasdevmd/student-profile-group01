...
public class SubjectService {
	...
	public ArrayList<Subjects> getSubjects(int userID) {
		...
		try {
			...
			query = "select s.subjectCode,s.subjectName,f.firstName,f.lastName,e.semester,e.status,e.grade from "
					...
					+ "and r.subjectID=e.subjectID and r.facultyID=f.facultyID and e.userID="
					+ userID
					+ " union select s.subjectCode,s.subjectName,f.firstName,f.lastName,s.semester,'-','-' from subjectdetails s,facultydetails f,enrollmentrules r where r.facultyID=f.facultyID and s.subjectID not in ( select s.subjectID from subjectdetails s,subjectenrollment e,facultydetails f,enrollmentrules r where s.subjectID=e.subjectID and r.subjectID=e.subjectID and r.facultyID=f.facultyID and e.userID=1 ) ";
	...
	public ArrayList<Subjects> getEnrolledSubjects(String status) {
		...
		try {
			...
			query = "select s.subjectCode,s.subjectName,f.firstName,f.lastName,e.semester,e.status,e.grade from "
					+ "subjectdetails s,subjectenrollment e,facultydetails f,enrollmentrules r where s.subjectID=e.subjectID "
					+ "and r.subjectID=e.subjectID and r.facultyID=f.facultyID and e.status='"
					+ status + "';";
